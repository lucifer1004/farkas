...
- *live-REPL*: one persistent REPL per worker, Mathlib imported once, each
  ...
  (e.g. proof-state snapshotting, arXiv:2605.25556) expose. 4 workers,
  REPL restart every 40 theorems.

## 2. Oracle replacement

The Rust daemon (`farkas-oracled`) reimplements Mathlib's simplex
certificate search behind the `CertificateOracle` interface — pluggable
by design; Lewis & Wu demonstrated a Mathematica-backed oracle for the
Lean 3 predecessor of this interface (arXiv:2101.07758) — over tiered
exact rationals (i64 → i128 → BigInt with
checked promotion; 99.94% of arithmetic operations remain in the i64 tier
on the reference corpus). Big-tier reductions use a division-based gcd:
simplex row operations produce operands with large shared factors, on
which binary (Stein) gcd degrades ~50×.

| metric (reference corpus) | stock (interpreted) | native daemon |
|---|---|---|
| oracle call, median | 4.18 ms | 0.43 ms |
| single-thread throughput (microbenchmark, all 4,599 calls) | 1× | 127.6× |
