...
fn norm_m(n: i128, d: i128) -> TRat {
    ...
    if n == i128::MIN || d == i128::MIN {
        ...
        return make_big(BigInt::from(n), BigInt::from(d));
    }
...
/// gcd via a division-based algorithm (dashu's), through a cheap
/// little-endian byte round-trip. num-bigint's own gcd is Stein's binary
/// algorithm, which degrades ~50x on the correlated operands that simplex
/// row operations produce (rational row scaling shares huge factors); a
/// division-based gcd collapses those in a few steps. Measured on the
/// 607-digit corpus coefficients: 13.5us -> 0.27us per gcd (correlated),
/// 50us -> 7us (random), conversions included.
fn big_gcd(a: &BigInt, b: &BigInt) -> BigInt {
    use dashu_int::{UBig, ops::Gcd};
    let x = UBig::from_le_bytes(&a.magnitude().to_bytes_le());
    let y = UBig::from_le_bytes(&b.magnitude().to_bytes_le());
    BigInt::from(BigUint::from_bytes_le(&(&x).gcd(&y).to_le_bytes()))
}

/// Reduced TRat from a raw numerator/denominator pair (`den != 0`):
/// sign-normalize, reduce via `big_gcd`, and demote. Replaces
/// `BigRational::new` (whose implicit reduction is the Stein gcd above)
/// on every Big-tier construction site.
fn make_big(mut num: BigInt, mut den: BigInt) -> TRat {
    if num.is_zero() {
        return TRat::S(0, 1);
    }
    if den.is_negative() {
    ...
    shrink_big(BigRational::new_raw(num, den))
}
    ...
    fn addsub(&self, o: &TRat, subtract: bool) -> TRat {
        match (self, o) {
            ...
            (TRat::B(_), _) | (_, TRat::B(_)) => {
                ...
                let t1 = x.numer() * y.denom();
                let t2 = y.numer() * x.denom();
                let n = if subtract { t1 - t2 } else { t1 + t2 };
                make_big(n, x.denom() * y.denom())
            }
            _ => {
                bump(1);
                ...
                let n = if subtract { t1 - t2 } else { t1 + t2 };
                make_big(n, BigInt::from(b) * BigInt::from(d))
            }
        }
    }
    ...
    pub fn mul(&self, o: &TRat) -> TRat {
        match (self, o) {
            ...
            (TRat::B(_), _) | (_, TRat::B(_)) => {
                ...
                let x = self.to_bigrational();
                let y = o.to_bigrational();
                // cross-reduce (mirrors the S tier): the shared factors live
                // between each numerator and the OPPOSITE denominator, and
                // the result is reduced by construction — no product-sized
                // final gcd needed
                let g1 = big_gcd(x.numer(), y.denom());
                let g2 = big_gcd(y.numer(), x.denom());
                let n = (x.numer() / &g1) * (y.numer() / &g2);
                let d = (x.denom() / &g2) * (y.denom() / &g1);
                if n.is_zero() {
                    TRat::S(0, 1)
                } else {
                    shrink_big(BigRational::new_raw(n, d))
                }
            }
            _ => {
                bump(1);
